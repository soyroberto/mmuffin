# Enhanced Music Recommendation System with Fixed Artist Search Button
# This version fixes the "More Like This" button in artist search results
...
def render_artist_search(df: pd.DataFrame):
    """Render artist search functionality with fixed 'More Like This' button"""
    ...
    with col2:
        if st.button("🗑️ Clear Results", key="clear_search_button"):
            ...
            st.session_state.search_query = ""
            st.session_state.search_performed = False
            st.session_state.hover_recommendations = []  # Also clear hover recommendations
            st.rerun()
    ...
    if st.session_state.search_performed and st.session_state.search_results:
        ...
        for result in st.session_state.search_results:
            ...
            with col2:
                # Fixed button with unique key and proper session state handling
...
def render_hover_recommendations():
    """Render hover recommendations if available"""
...
def render_more_like_this_buttons(tier_artists_df: pd.DataFrame):
    """Render 'More Like This' buttons for tier artists"""
    if tier_artists_df.empty:
        return
