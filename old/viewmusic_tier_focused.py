#!/usr/bin/env python3
"""
🎵 Personal Music Recommendation System - Tier-Focused Streamlit Web Application

This is a robust web interface for the hybrid AI/ML music recommendation system.
Features automatic data loading with proper session state management, tier range visualization with ranks, and tier-focused data overview.

TIER-FOCUSED VERSION:
- Artist ranks displayed on the left side of the chart
- Data overview metrics reflect only the selected tier range
- Visual tier range highlighting in green/gray maintained
- Sidebar unchanged (left side interface preserved)
- Real-time updates of metrics based on tier selection
...
def render_main_header():
    ...
    <div style="text-align: center; margin-bottom: 2rem;">
        <p style="font-size: 1.2rem; color: #666;">
            Tier-Focused Version - Artist Ranks & Tier-Based Data Overview
        </p>
    </div>
    """, unsafe_allow_html=True)

def create_tier_focused_chart(df: pd.DataFrame, tier_start: int, tier_end: int):
    """Create tier-focused chart with artist ranks on the left"""
    try:
        # Get top 30 artists for better visualization
        ...
        # Determine which artists are in the selected tier range
        tier_start_actual = min(tier_start, tier_end)
        tier_end_actual = max(tier_start, tier_end)
        
        # Create color mapping based on tier selection
        colors = []
        for rank in top_artists['rank']:
            if tier_start_actual <= rank <= tier_end_actual:
                colors.append('#1DB954')  # Spotify green for selected tier
            else:
                colors.append('#666666')  # Gray for non-selected
        
        # Create artist labels with ranks on the left
        artist_labels = [f"#{rank}: {artist}" for rank, artist in zip(top_artists['rank'], top_artists['artist'])]
        ...
        fig.add_trace(go.Bar(
            x=top_artists['hours_played'],
            y=artist_labels,  # Use rank-prefixed labels
            orientation='h',
            marker=dict(
                color=colors,
            ...
            hovertemplate='<b>%{customdata}</b><br>' +
                         ...
                         'Rank: #%{text}<br>' +
                         '<extra></extra>',
            customdata=top_artists['artist'],  # Original artist names for hover
            text=top_artists['rank'],  # Rank numbers for hover
            name='Artists'
        ))
        
        # Add tier range highlighting with annotations
        if tier_start_actual <= 30 and tier_end_actual >= 1:  # Only if tier range is visible
            # Add a subtle background highlight for the tier range
            tier_artists_in_view = top_artists[
                (top_artists['rank'] >= tier_start_actual) & 
                (top_artists['rank'] <= tier_end_actual)
            ]
            
            if len(tier_artists_in_view) > 0:
                # Add annotation for tier range
                fig.add_annotation(
                    x=top_artists['hours_played'].max() * 0.7,
                    y=len(top_artists) - 1,
                    text=f"🎯 Selected Tier: #{tier_start_actual}-#{tier_end_actual}<br>" +
                         f"📊 {len(tier_artists_in_view)} artists highlighted in green",
                    showarrow=True,
                    arrowhead=2,
                    arrowsize=1,
                    arrowwidth=2,
        ...
        fig.update_layout(
            title=dict(
                text="🏆 Top 30 Artists by Listening Hours<br><sub>Artist Ranks on Left • Green = Selected for Recommendations</sub>",
                x=0.5,
                font=dict(size=16)
            ),
...
def render_data_overview_and_recommendations(config):
    ...
    # Enhanced top artists chart with tier highlighting and ranks
    st.markdown("### 🏆 Top Artists with Tier Range Visualization")
    
    # Create and display the tier-focused chart
    tier_chart = create_tier_focused_chart(
        df, 
        config['tier_start'], 
        config['tier_end']
    )
    ...
    try:
        export_data = {
            'export_metadata': {
                ...
                'export_type': 'music_recommendations_tier_focused',
                'system_version': '2.7_streamlit_tier_focused_data_overview'
            },
            'recommendations': st.session_state.recommendations,
...
def main():
    ...
    with tab3:
        st.markdown("""
        ## 🎵 About This System (Tier-Focused Version)
        
        This is a **Tier-Focused Hybrid AI/ML Music Recommendation System** with artist ranks displayed and tier-based data overview.
        
        ### 🆕 Latest Enhancements
        
        1. **Artist Ranks on Chart**: Artist ranks (#1, #2, etc.) are now displayed on the left side of the chart
        2. **Tier-Based Data Overview**: Metrics now reflect only the selected tier range
        3. **Green Metric Cards**: Selected tier metrics are highlighted in green
        4. **Enhanced Chart Labels**: Clear rank-based labeling for better navigation
        5. **Real-Time Tier Updates**: Data overview updates immediately when tier range changes
        
        ### 🎯 Visual Features
        
        - **Rank Display**: "#1: Artist Name" format shows exact rankings
        - **Tier-Focused Metrics**: Green cards show data only from selected artists
        - **Green/Gray Logic**: Maintained visual highlighting of selected vs non-selected artists
        - **Left Sidebar Unchanged**: All controls remain exactly as before
        - **Real-Time Updates**: All metrics update immediately when you change tier settings
        
        ### 📊 Data Overview Enhancement
        
        **Green Cards (Tier-Specific):**
        - **Tier Plays**: Total plays from artists in your selected range
        - **Tier Artists**: Number of artists in your selected range  
        - **Tier Hours**: Total listening hours from selected artists
        
        **Regular Cards (Overall):**
        - **Date Range**: Overall date range of your data
        ...
        4. **Visual Tier Selection**: See exactly which artists will be used for recommendations
        5. **Tier-Based Analytics**: Data overview reflects your selected artist range
        6. **Enhanced Recommendations**: Up to 50 recommendations with improved variety
        7. **Robust Operation**: No crashes on any interaction
        
        ### 🧠 AI/ML Engines
        
        1. **Content-Based Filtering**: Last.fm API + Artist Similarity + Visual Tier Selection
        ...
        4. **Artist Listing & Ranking**: Preference Modeling + Rank-Based Visual System
        
        ### 📁 Required File Structure
        
        ```
        your-project/
        ├── viewmusic.py                    # This tier-focused app
        ...
        2. **Visual Tier Selection**: See artist ranks (#1, #2, etc.) and select your range
        3. **Real-Time Analytics**: Green metric cards show data for your selected tier
        4. **Generate Recommendations**: Click "Recommend" for up to 50 AI-powered suggestions
        ...
        ### ✅ Tier-Focused Features
        
        - **✅ Artist ranks displayed on chart (#1, #2, etc.)**
        - **✅ Tier-based data overview with green highlighting**
        - **✅ Real-time metric updates based on tier selection**
        - **✅ Enhanced chart with rank-based labels**
        - **✅ Sidebar unchanged (left interface preserved)**
        - **✅ Green/gray selection logic maintained**
        - **✅ All previous fixes and enhancements preserved**
        
        ---
        
        **Created by**: Roberto's AI Music Recommendation System  
        **Version**: 2.7 (Tier-Focused - Ranks & Tier-Based Data Overview)  
        **GitHub**: [soyroberto/streamlit](https://github.com/soyroberto/streamlit)  
        **Status**: Production Ready - Enhanced Tier Analytics
        """)
