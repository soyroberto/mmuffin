#!/usr/bin/env python3
"""
🎵 Personal Music Recommendation System - Enhanced Streamlit Web Application

This is a robust web interface for the hybrid AI/ML music recommendation system.
Features automatic data loading with proper session state management, tier range visualization, and enhanced recommendations.

ENHANCED VERSION:
- Visual tier range highlighting in the top artists graph
- Increased max recommendations to 50
- Real-time visual feedback on selected artist tiers
...
def render_main_header():
    ...
    <div style="text-align: center; margin-bottom: 2rem;">
        <p style="font-size: 1.2rem; color: #666;">
            Enhanced Version - Visual Tier Range & Max 50 Recommendations
        </p>
    </div>
    """, unsafe_allow_html=True)

def create_enhanced_top_artists_chart(df: pd.DataFrame, tier_start: int, tier_end: int):
    """Create enhanced top artists chart with tier range highlighting"""
    try:
        ...
        fig.add_trace(go.Bar(
            ...
            text=[f"#{rank}" for rank in top_artists['rank']],
            textposition='inside',
        ...
        if tier_start_actual <= 30 and tier_end_actual >= 1:  # Only if tier range is visible
            ...
            if len(tier_artists_in_view) > 0:
                ...
                fig.add_annotation(
                    ...
                    y=len(top_artists) - 1,
                    text=f"🎯 Selected Tier Range: {tier_start_actual}-{tier_end_actual}<br>" +
        ...
        fig.update_layout(
            title=dict(
                text="🏆 Top 30 Artists by Listening Hours<br><sub>Green = Selected Tier Range for Recommendations</sub>",
                x=0.5,
                font=dict(size=16)
            ),
...
def render_data_overview_and_recommendations(config):
    ...
    # Enhanced top artists chart with tier highlighting
    st.markdown("### 🏆 Top Artists with Tier Range Visualization")
    
    # Create and display the enhanced chart
    enhanced_chart = create_enhanced_top_artists_chart(
        df, 
        config['tier_start'], 
        config['tier_end']
    )
    ...
    if st.session_state.artist_rankings is not None:
        ...
        if len(tier_artists) > 0:
            ...
            with col2:
                st.info(f"📊 **Artists in Range:** {len(tier_artists)}")
            
            with col3:
                total_hours = tier_artists['total_hours'].sum()
                st.info(f"⏱️ **Total Hours:** {total_hours:.1f}h")
    ...
    try:
        export_data = {
            'export_metadata': {
                ...
                'export_type': 'music_recommendations_enhanced',
                'system_version': '2.6_streamlit_enhanced_tier_visualization'
            },
            'recommendations': st.session_state.recommendations,
...
def main():
    ...
    with tab3:
        st.markdown("""
        ## 🎵 About This System (Enhanced Version)
        
        This is an **Enhanced Hybrid AI/ML Music Recommendation System** with visual tier range highlighting and increased recommendation capacity.
        
        ### 🆕 Latest Enhancements
        
        1. **Visual Tier Range Highlighting**: Top artists chart now highlights your selected tier range in green
        2. **Increased Max Recommendations**: Now supports up to 50 recommendations (increased from 100)
        3. **Real-Time Tier Feedback**: Sidebar shows which artists will be used for recommendations
        4. **Enhanced Chart Annotations**: Visual indicators show exactly which artists are selected
        5. **Improved Performance**: Cached artist rankings for faster interactions
        
        ### 🎯 Visual Features
        
        - **Green Highlighting**: Artists in your selected tier range are highlighted in green on the chart
        - **Tier Information**: Sidebar shows top and bottom artists in your selected range
        - **Interactive Annotations**: Chart annotations point to your selected tier range
        - **Real-Time Updates**: Visual feedback updates immediately when you change tier settings
        ...
        5. **Enhanced Recommendations**: Up to 50 recommendations with improved variety
        ...
        4. **Artist Listing & Ranking**: Preference Modeling + Visual Ranking System
        
        ### 📁 Required File Structure
        
        ```
        your-project/
        ├── viewmusic.py                    # This enhanced app
        ...
        2. **Visual Tier Selection**: See highlighted artists in the chart as you adjust tier range
        3. **Real-Time Feedback**: Sidebar shows exactly which artists will be used
        ...
        - **✅ Visual tier range highlighting in green**
        - **✅ Real-time feedback on selected artists**
        - **✅ Increased max recommendations to 50**
        ...
        **Version**: 2.6 (Enhanced - Visual Tier Range & Max 50 Recs)  
        **GitHub**: [soyroberto/streamlit](https://github.com/soyroberto/streamlit)  
        **Status**: Production Ready - Enhanced User Experience
        """)
